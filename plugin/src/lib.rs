...
pub fn main() {
    ...
    install_hooks!(
        ...
        mapsequencehuman_postmoveunit_hook,
        mapsequencehuman_waitmovingunit_hook,
        mapsequencemind_fixed_hook,
        unit_createforsummonimpl1_hook,
        unitutil_summondelete_hook,
        unitutil_unitupdpate_hook,
    );
    ///////////////////////////
    // Always Use Simple Summon
    // 71023cf8e8 c0 00 00 36 tbz __this,#0x0,LAB_71023cf900 => 06 00 00 14 b LAB_71023cf900
    // Patch::in_text(0x023cf8e8).bytes(&[0x06, 0x00, 0x00, 0x14]).unwrap();

    // SUMMON DELETE PATCHES:
    // Update:
    // 7101c77eec 00 e8 07 36 tbz method,#0x0,LAB_7101c77bec
    // Patch::in_text(0x01c77eec).bytes(&[0x00, 0xe8, 0x07, 0x36]).unwrap();
    // Update (Just remove all summon delete)
    // 7101c77bf0 8c fe ff 97 bl App.UnitUtil$$SummonDeleteImpl => nop 1f 20 03 d5
    Patch::in_text(0x01c77bf0).bytes(&[0x1f, 0x20, 0x03, 0xd5]).unwrap();
...
pub extern "C" fn disengage_get_mind(_this: &(), _method_info: OptionalMethod) -> i32 {
    ...
    // Using this one seems to cause the Engage Menu Command to just enter Engage Mode.
...
///////////////////////////////////////
#[unity::from_offset("App", "MapSequenceMind", "Fixed")]
fn mapsequencemind_fixed(this: &MapSequenceMind, method_info: OptionalMethod);
#[unity::from_offset("App", "MapSequenceMind", "DoneAction")]
fn mapsequencemind_doneaction(this: &MapSequenceMind, method_info: OptionalMethod);
/////////////////////////////////////////

// 0x7102680630
// void App.MapSequenceMind$$Fixed(App_MapSequenceMind_o *__this,MethodInfo *method)
#[skyline::hook(offset = 0x02680630)]
pub fn mapsequencemind_doneaction_hook(this: &MapSequenceMind, _method_info: OptionalMethod) {
    println!("[mapsequencemind_doneaction_hook/BEG]");
    call_original!(this, _method_info);
    println!("[mapsequencemind_doneaction_hook/END]");
}


// 0x7102680f80
// void App.MapSequenceMind$$Fixed(App_MapSequenceMind_o *__this,MethodInfo *method)
#[skyline::hook(offset = 0x02669b00)]
pub fn mapsequencemind_fixed_hook(this: &MapSequenceMind, _method_info: OptionalMethod) {
    println!("[mapsequencemind_fixed_hook/BEG]");
    call_original!(this, _method_info);
    println!("[mapsequencemind_fixed_hook/END]");
}
...
#[unity::from_offset("App", "MapSequenceHuman", "PostMoveUnit")]
fn mapsequencehuman_postmoveunit(this: &MapSequenceHuman, method_info: OptionalMethod);
#[unity::from_offset("App", "MapSequenceHuman", "WaitMovingUnit")]
fn mapsequencehuman_waitmovingunit(this: &MapSequenceHuman, method_info: OptionalMethod);
//////////////////////////////////////////////////////////////////////

// 0x7102669a50
// void App.MapSequenceHuman$$WaitMovingUnit(App_MapSequenceHuman_o *__this,MethodInfo *method)
#[skyline::hook(offset = 0x02669a50)]
pub fn mapsequencehuman_waitmovingunit_hook(this: &MapSequenceHuman, _method_info: OptionalMethod) {
    println!("[mapsequencehuman_waitmovingunit_hook/BEG]");
    call_original!(this, _method_info);
    println!("[mapsequencehuman_waitmovingunit_hook/END]");
}

// 0x7102669b00
// void App.MapSequenceHuman$$PostMoveUnit(App_MapSequenceHuman_o *__this,MethodInfo *method)
#[skyline::hook(offset = 0x02669b00)]
pub fn mapsequencehuman_postmoveunit_hook(this: &MapSequenceHuman, _method_info: OptionalMethod) {
    println!("[mapsequencehuman_postmoveunit_hook/BEG]");
    call_original!(this, _method_info);
    println!("[mapsequencehuman_postmoveunit_hook/END]");
}
...
pub fn mapsequencehuman_createbind(sup: &mut MapSequence, is_resume: bool, _method_info: OptionalMethod) {
    ...
    vec.insert(0x9a, desc);
    // This one might not do anything //
    ...
    // ItemMenuEngageAttack:: 21 :: 0x13
    let desc = engage::proc::desc::ProcDesc::jump(0x15);
    // UnitCommand = 16;
    // let desc = engage::proc::desc::ProcDesc::jump(0x10);
    // SortiePositionChange = 16;
    // let desc = engage::proc::desc::ProcDesc::jump(0xd);
    vec.insert(0x9a, desc);
    ...
    //
    let method = mapsequencemind_doneaction::get_ref();
    let method = unsafe { std::mem::transmute(method.method_ptr) };
    let desc = unsafe { ProcDesc::call(ProcVoidMethodMut::new(&mut (*sup.child), method)) };
    vec.insert(0x9a, desc);
    /////////////////////////////////////
    // let method = mapsequencehuman_waitmovingunit::get_ref();
    // let method = unsafe { std::mem::transmute(method.method_ptr) };
    // let desc = unsafe { ProcDesc::call(ProcVoidMethodMut::new(&mut (*sup.child), method)) };
    // vec.insert(0x9a, desc);
    //////////////////////////////////////////////////////////////////
    // // Greys out unit but does not add entry to map history
    // let method = mapsequencemind_fixed::get_ref();
    // let method = unsafe { std::mem::transmute(method.method_ptr) };
    // let desc = unsafe { ProcDesc::call(ProcVoidMethodMut::new(&mut (*sup.child), method)) };
    // vec.insert(0x9a, desc);
    ///////
    // This is required, otherwise it crashes even earlier.
...
pub fn mapsequencetargetselect_decide_normal(this: &mut MapSequenceTargetSelect, _method_info: OptionalMethod) {
    ...
    if cur_mind == DISENGAGE_MIND_TYPE {
        ...
        let _mapmind_instance = get_instance::<MapMind>();
        ...
        // Restore Combat Summon?
        Patch::in_text(0x023cf8e8).bytes(&[0xc0, 0x00, 0x00, 0x36]).unwrap();
...
pub fn mapsequenceengagesummon_simplesummon_hook(this: &MapSequenceEngageSummon, _method_info: OptionalMethod) {
    ...
    let summoner = MapMind::get_unit();
    println!("[mapsequenceengagesummon_simplesummon_hook] summoner: {} ?", summoner.get_pid());
...
pub fn unit_createforsummonimpl1_hook(this: &Unit, person: Option<&PersonData>, original: Option<&Unit>, rank: i32, _method_info: OptionalMethod) {
    println!("[unit_createforsummonimpl1_hook/BEG]");
    if let Some(person_unwrapped) = person {
        if let Some(original_unwrapped) = original {
            println!("[unit_createforsummonimpl1_hook] person: {} original: {}", Mess::get(person_unwrapped.get_name().unwrap()).to_string(), original_unwrapped.get_pid());
            call_original!(this, person, original, rank, _method_info);
        } else {
            println!("[unit_createforsummonimpl1_hook] there is no original?");
        }
    } else {
        println!("[unit_createforsummonimpl1_hook] no person??? Is this a problem?");
    }

    println!("[unit_createforsummonimpl1_hook/END]");
}


// 0x7101c77570
// void App.UnitUtil$$SummonDelete(App_Unit_o *owner,MethodInfo *method)
#[skyline::hook(offset = 0x01c77570)]
pub fn unitutil_summondelete_hook(owner: Option<&Unit>, _method_info: OptionalMethod) {
    println!("[unitutil_summondelete_hook/BEG]");
    if let Some(owner_unwrapped) = owner {
        println!("[unitutil_summondelete_hook] owner: {}", owner_unwrapped.get_pid());
    }
    call_original!(owner, _method_info);
    println!("[unitutil_summondelete_hook/END]");
}

// 0x7101c77800
// void App.UnitUtil$$UnitUpdate(MethodInfo *method)
#[skyline::hook(offset = 0x01c77800)]
pub fn unitutil_unitupdpate_hook(_method_info: OptionalMethod) {
    println!("[unitutil_unitupdpate_hook/BEG]");
    call_original!(_method_info);
    println!("[unitutil_unitupdpate_hook/END]");
}
